...
print(f"Testing cover_image as TextField...")
print(f"URL length: {len(test_url)}")

try:
    # Create a test story
    story = Story.objects.create(
        title="Test Story with Long URL",
        author=user,
        content="Test content",
        canvas_data="[]",
        category="fantasy",
        language="en",
        cover_image=test_url
    )
    
    print(f"✅ Story created successfully with ID: {story.id}")
    print(f"✅ Cover image URL saved (length: {len(story.cover_image)})")
    print(f"✅ Cover image preview: {story.cover_image[:100]}...")
    
    # Clean up
    story.delete()
    print(f"✅ Test story deleted")
